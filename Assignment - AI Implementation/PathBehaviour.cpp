...
eBehaviourResult PathBehaviour::execute(GameObject * gameObject, float deltaTime)
{
	...
	if (m_path.path.empty() == false) {//checking if path is not empty
		if (m_pos.pythag(m_path.path.top()) < m_speed) {//checking if next node is within one frames movment of gameObject
			Vector2 target_dir = m_path.path.top() - m_pos;
